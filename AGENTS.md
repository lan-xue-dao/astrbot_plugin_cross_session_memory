...
- 核心导入：`from astrbot.api.event import filter, AstrMessageEvent`
- 插件基类：`from astrbot.api.star import Context, Star, register`
- LLM 相关：`from astrbot.api.provider import ProviderRequest, LLMResponse`
- 配置和日志：`from astrbot.api import logger, AstrBotConfig`

### 日志规范

- 使用 `astrbot.api.logger` 记录日志
- 日志前缀使用 `[跨会话记忆]` 便于识别
- 重要操作记录 INFO 级别，调试信息记录 DEBUG 级别
- 错误信息使用 ERROR 级别

### 命名规范

- 类名使用 PascalCase（如 `MemoryGroup`）
- 函数和方法名使用 snake_case（如 `on_llm_request`）
- 私有方法以下划线开头（如 `_save_data`）
- 常量使用大写下划线命名（如有）

### 装饰器使用

- 插件注册：使用 `@register()` 装饰器注册插件
- 命令注册：使用 `@filter.command("command_name")` 注册命令
- 事件钩子：
